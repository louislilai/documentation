...
**Purchase Tax**, and click on :guilabel:`Save`.
...
--> Configuration --> Taxes` and use the :guilabel:`Activate` toggle button to activate a tax.
...
The **tax scope** determines the tax application, which also restricts where it is displayed.
...
The label of the tax as displayed on each invoice line in the :guilabel:`Taxes` column. This is the
:ref:`label <taxes/labels>` visible to *front-end* users, on exported invoices, in customer portals,
etc.
